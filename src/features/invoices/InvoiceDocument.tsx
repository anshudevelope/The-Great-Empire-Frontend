...
export function InvoiceDocument({ invoice }: { invoice: Invoice }) {
  ...
  return (
    <article className="print-area mx-auto w-full max-w-[820px] bg-white p-8 text-text sm:p-10">
      ...
      <section className="grid gap-6 border-b border-border py-6 sm:grid-cols-2">
        {/* LEFT COLUMN: Member Details (Whom the payment is for) */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-text-subtle">For Member</p>
          {transaction.forMember ? (
            <div className="mt-2 space-y-1">
              <p className="font-medium text-text">
                {transaction.forMember.name ?? 'N/A'}
                <span className="ml-2 font-mono text-sm text-text-muted">
                  ({transaction.forMember.memberCode})
                </span>
              </p>

              {transaction.forMember.address && (
                <p className="max-w-xs text-sm text-text-muted">{transaction.forMember.address}</p>
              )}

              {(transaction.forMember.phone || transaction.forMember.email) && (
                <p className="text-sm text-text-muted">
                  {[transaction.forMember.phone, transaction.forMember.email].filter(Boolean).join(' · ')}
                </p>
              )}
            ...
            <p className="mt-2 text-sm text-text-muted">No specific member associated</p>
          )}
        </div>

        {/* RIGHT COLUMN: Received from (Payer) & Transaction Details */}
        <div className="sm:text-right">
          <p className="text-xs font-semibold uppercase tracking-wide text-text-subtle">Received From (Payer)</p>
          <p className="mt-2 font-medium">{billedTo.name}</p>
          {billedTo.memberCode && (
            <p className="font-mono text-sm text-text-muted">{billedTo.memberCode}</p>
          )}
          {/* {billedTo.address && (
            <p className="mt-1 text-sm text-text-muted sm:ml-auto sm:max-w-xs">{billedTo.address}</p>
          )}
          <p className="mt-1 text-sm text-text-muted">
            {[billedTo.phone, billedTo.email].filter(Boolean).join(' · ')}
          </p> */}
